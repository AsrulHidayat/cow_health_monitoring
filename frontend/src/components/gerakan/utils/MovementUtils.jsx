...
const groupByTimeInterval = (data, minutesRange, intervalType, isDateRangeActive = false) => {
  ...
  relevantData.forEach(item => {
    ...
    if (!grouped[key]) grouped[key] = { movements: [], date: dt };
    const t = Number(item.movement);
    if (!isNaN(t)) grouped[key].movements.push(t);
  });

  const result = Object.values(grouped).map(group => {
    const movements = group.movements;
    const avg = movements.length ? movements.reduce((s, v) => s + v, 0) / movements.length : null;
    const minMovement = movements.length ? Math.min(...movements) : null;
    const maxMovement = movements.length ? Math.max(...movements) : null;
    ...
    return {
      time: timeLabel,
      movement: avg !== null ? parseFloat(avg.toFixed(1)) : null,
      minMovement: minMovement !== null ? parseFloat(minMovement.toFixed(1)) : null,
      maxMovement: maxMovement !== null ? parseFloat(maxMovement.toFixed(1)) : null,
      fullDate: date,
  ...
  switch (timePeriod) {
    ...
    case TIME_FILTERS.MINUTE.value: {
      ...
      filtered.forEach(item => {
        ...
        grouped[key].push(Number(item.movement));
      });

      return Object.entries(grouped).map(([minuteKey, movements], index) => {
        const avgMovement = movements.reduce((a, t) => a + t, 0) / movements.length;
        return {
          index: index + 1,
          displayIndex: `#${index + 1}`,
          time: minuteKey,
          movement: parseFloat(avgMovement.toFixed(1)),
...
// 🔹 KATEGORI GERAKAN
// ========================================================
export const categorizeMovement = (movement) => {
  if (movement == null) {
    return { label: 'N/A', color: 'gray', value: 'na' };
  }
  if (movement < 20) {
    return { label: 'Istirahat', color: 'blue', value: 'istirahat' };
  } else if (movement >= 20 && movement <= 60) {
    return { label: 'Normal', color: 'green', value: 'normal' };
  } else if (movement > 60) {
    return { label: 'Aktif', color: 'yellow', value: 'aktif' };
  }
  return { label: 'N/A', color: 'gray', value: 'na' };
};
// ========================================================
